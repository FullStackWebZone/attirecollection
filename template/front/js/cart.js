...
function cart_submission(elem) {
  ...
  if ($(".delivery_address .country").val() == "0") {
    countryok = "no";
    $(".country .selectpicker").css("border-color", "red");
  }
  if (okay == "yes" && countryok == "yes") {
    var payment_type = "";
    var state = check_login_stat("state");
    if ($("input[name='payment_type']:checked").val() == "benefitApp") {
      redirectobenefit();
    } else {
...
function countryShip(val) {
  var url = base_url + "home/cart/calcs/full?country=" + val;
  var total = $("#total");
  var ship = $("#shipping");
  var tax = $("#tax");
  var grand = $("#grand");
...
function set_cart_map() {
  ...
  geocoder.geocode({ address: address }, function (results, status) {
    /*var country_sec = "";
			var ssss = results[0].address_components;
			for(var i = 0; i<ssss.length; i++){
				if(ssss[i].types[0] =='country'){
					country_sec = ssss[i].long_name;
				}
			}
			*/
