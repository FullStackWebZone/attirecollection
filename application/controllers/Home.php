...
class Home extends CI_Controller
{
    ...
    {
        $data['total_count'] = $this->crud_model->getUserVisitorCount();

        if ($data['total_count'][0]['count'] == '') {
            $count = '1';
        } else {
            $count = $data['total_count'][0]['count'] + 1;
        }

        $res = $this->crud_model->updateData('visitor_count', array('count' => $count, 'updated_at' => date('Y-m-d H:i:s')), array('id' => '1'));
        //$this->output->enable_profiler(TRUE);
    ...
    function category($para1 = "", $para2 = "", $min = "", $max = "", $text = '')
    {
        ...
        $sub_sub_category_id = $this->uri->segment(5);
        $fourth_sub_category_id = $this->uri->segment(6);
        ...
        if (!empty($category_id) && !empty($sub_category_id) && !empty($sub_sub_category_id)) {
        ...
        if (!empty($category_id) && !empty($sub_category_id) && !empty($sub_sub_category_id) && !empty($fourth_sub_category_id)) {

            $page_data['all_products'] = $this->db->get_where('product', array(
                'category' => $category_id,
                'sub_category' => $sub_category_id,
                'sub_sub_category' => $sub_sub_category_id,
                'fourth_sub_category' => $fourth_sub_category_id,
            ))->result_array();
        }
        ...
        $page_data['cur_brand'] = $brand;
        $page_data['cur_category'] = $para1;
        $page_data['cur_fourth_sub_category'] = $fourth_sub_category_id;
        $page_data['cur_sub_sub_category'] = $sub_sub_category_id;
        $page_data['cur_category'] = $para1;
        $page_data['text'] = $text;
    ...
    {
        $page_data['page_name'] = "others/return_policy";
        $page_data['asset_page'] = "all_category";
        $page_data['page_title'] = translate('return_policy');
        $page_data['faqs'] = json_decode($this->crud_model->get_type_name_by_id('business_settings', '11', 'value'), true);
        $this->load->view('front/index', $page_data);
    }
    ...
    function listed($para1 = "", $para2 = "", $para3 = "")
    {
        ...
        if ($para1 == "click") {
            ...
            $sub_category = $this->input->post('sub_category');
            $sub_category = !empty($sub_category) ? explode(',', $sub_category) : $sub_category;


            $sub_sub_category = $this->input->post('sub_sub_category');
            $sub_sub_category = !empty($sub_sub_category) ? explode(',', $sub_sub_category) : $sub_sub_category;

            $fourth_sub_category = $this->input->post('fourth_sub_category');
            $fourth_sub_category = !empty($fourth_sub_category) ? explode(',', $fourth_sub_category) : $fourth_sub_category;
            ...
            $query = array();
            /*
			if (count($sub_fourth_category) > 0) {
                $i = 0;
                foreach ($sub_fourth_category as $row2) {
                    $i++;
                    if ($row !== "") {
                        if ($row2 !== "0") {
                            $query[] = $row2;
                            ...
                            $this->db->where('sub_fourth_category !=', '0');
                        }
                    }
                }
                if ($setter == 'get') {
					print_r($query);
                    $this->db->where_in('sub_fourth_category', $query);
                }
            }
			*/
            if (!empty($category)) {
            ...
            if (is_array($fourth_sub_category) && count($fourth_sub_category) > 0) {

                $this->db->or_where_in('fourth_sub_category', $fourth_sub_category);
            } else {

                if (!empty($fourth_sub_category)) {

                    $this->db->or_where('fourth_sub_category', $fourth_sub_category);
                }
            }
            /*
			if (count($sub_sub_category) > 0) {
                $i = 0;
                foreach ($sub_sub_category as $row1) {
                    $i++;
                    if ($row !== "") {
                        if ($row1 !== "0") {
                            $query[] = $row1;
                            ...
                            $this->db->where('sub_sub_category !=', '0');
                        }
                    }
                }
                if ($setter == 'get') {
					
                    $this->db->where_in('sub_sub_category', $query);
                }
            }
			*/

            if (is_array($sub_sub_category) && count($sub_sub_category) > 0) {

                $this->db->Or_where_in('sub_sub_category', $sub_sub_category);
            } else {

                if (!empty($sub_sub_category)) {

                    $this->db->or_where('sub_sub_category', $sub_sub_category);
                }
            }
            ...
            if (is_array($sub_category) && count($sub_category) > 0) {
                ...
                $this->db->or_where_in('sub_category', $sub_category);
            } else {

                if (!empty($sub_category)) {

                    $this->db->or_where('sub_category', $sub_category);
                }
            }
            ...
            $query = array();
            $query2 = array();
            /*if (count($fourth_sub_category) > 0) {
                $i = 0;
                foreach ($fourth_sub_category as $row3) {
                    $i++;
                    if ($row !== "") {
                        if ($row !== "0") {
                            $query[] = $row3;
                            ...
                            $this->db->where('fourth_sub_category !=', '0');
                        }
                    }
                }
                if ($setter == 'get') {
                    $this->db->where_in('fourth_sub_category', $query);
                }
            }
			 ...
			 if (count($sub_sub_category) > 0) {
                $i = 0;
                foreach ($sub_sub_category as $row4) {
                    $i++;
                    if ($row !== "") {
                        if ($row !== "0") {
                            $query[] = $row4;
    ...
    {
        if (
            isset($_GET['product_affiliation_code'])
    ...
    function cart($para1 = '', $para2 = '', $para3 = '', $para4 = '')
    {
        ...
        if ($para1 == 'calcs') {
            ...
            foreach ($cartdata as $cart) {
                ...
                $product = $this->db->get_where('product', array('product_id' => $cart['id']))->row_array();

                $shipping_cost_type = $product['shipping_cost_type'];
                $bundle_shipping_cost = $product['bundle_shipping_cost'];
                $bundle_shipping_qty = $product['bundle_shipping_qty'];
                ...
                if ($shipping_cost_type == 'product_wise') {
                    ...
                    $ship_amount += $product['shipping_cost'] * $cart['qty'];
                }

                if ($shipping_cost_type == 'country_wise' && !empty($_GET['country'])) {
                    $shippingData = $this->db->get_where('country_shipping_charges', array(
                        'country_id' => $_GET['country'],
                        'product_id' => $cart['id']
                    ))->row();

                    $ship_amount += (int)$shippingData->shipping_cost * $cart['qty'];
                }

                if ($shipping_cost_type == 'product_country_wise') {
                    ...
                    $ship_amount += (int)$shippingData->shipping_cost +  ($product['shipping_cost'] * $cart['qty']);
                }
            }
            ...
            } elseif ($this->crud_model->get_type_name_by_id('business_settings', '3', 'value') == 'country_wise' && !empty($_GET['country'])) {
               
                $shipping = $this->db->get_where('country_shipping_charges',array(
                    'country_id' => $_GET['country']
                    ))->row()->shipping_cost;
                 // echo  $this->db->last_query();
                    $shipping = (int)$shipping == 0 ? 0 : $shipping;
            }*/
            $min_cart_amount    = $this->db->get_where('business_settings', array(
                'type' => 'min_cart_amount'
            ))->row()->value;
            $tax = $this->crud_model->cart_total_it('tax');
            if ((int)$min_cart_amount > 0) {
            ...
            if ($para2 == 'full') {
                $ship = $total < $min_cart_amount ? $shipping : '';
    ...
    function cart_checkout($para1 = "")
    {
        ...
        if ($para1 == "orders") {
            ...
            $this->load->view('front/shopping_cart/delivery_address', $data);
    ...
    function cart_finish($para1 = "", $para2 = "")
    {
        ...
        } else if ($this->input->post('payment_type') == 'razorpay') {
            if ($para1 == 'go') {
                ...
                $razorpay_key_id = $this->crud_model->get_settings_value('business_settings', 'razorpay_key_id', 'value');
                $razorpay_secret_key = $this->crud_model->get_settings_value('business_settings', 'razorpay_secret_key', 'value');
                $api = new Razorpay\Api\Api($razorpay_key_id, $razorpay_secret_key);
                ...
                $user_id = $this->session->userdata('user_id');
                /****TRANSFERRING USER TO PAYPAL TERMINAL****/
                $this->razorpay->add_field('key_id', $razorpay_key_id);
                $this->razorpay->add_field('order_id', $order->id);
                $this->razorpay->add_field('name', 'Attire Collection');


                if ($this->session->userdata('user_login') == 'yes') {
                    $this->razorpay->add_field('prefill[name]', $this->db->get_where('user', array('user_id' => $user_id))->row()->username);
                } else {
                    $info = json_decode($this->db->get_where('sale', array('sale_id' => $sale_id))->row()->shipping_address, true);
                    $this->razorpay->add_field('prefill[name]', $info['firstname']);
                }
                if ($this->session->userdata('user_login') == 'yes') {
                    $this->razorpay->add_field('prefill[email]', $this->db->get_where('user', array('user_id' => $user_id))->row()->email);
                } else {
                    $info = json_decode($this->db->get_where('sale', array('sale_id' => $sale_id))->row()->shipping_address, true);
                    $this->razorpay->add_field('prefill[email]', $info['email']);
                }
                if ($this->session->userdata('user_login') == 'yes') {
                    $this->razorpay->add_field('prefill[contact]', $this->db->get_where('user', array('user_id' => $user_id))->row()->phone);
                } else {
                    $info = json_decode($this->db->get_where('sale', array('sale_id' => $sale_id))->row()->shipping_address, true);
                    $this->razorpay->add_field('prefill[contact]', $info['phone']);
                }

                $this->razorpay->add_field('notes[sale_id]', $sale_id);

                $this->razorpay->add_field('callback_url', base_url() . 'home/razorpay_success');
                $this->razorpay->add_field('cancel_url', base_url() . 'home/razorpay_failure');

                // submit the fields to pum
                $this->razorpay->submit_razor_post();
            }
        ...
        } else if ($this->input->post('payment_type') == 'benefitPay') {
            if ($para1 == 'go') {
                ...
                $this->session->set_userdata('sale_id', $sale_id);
                $this->load->view('vw_subscription_home', $data);
            }
        } else if ($this->input->post('payment_type') == 'benefitApp') {
            if ($para1 == 'go') {
    ...
    /* FUNCTION: Loads after cancelling benefitPay*/
    function benefitPay_cancel()
    {
        $sale_id = $this->session->userdata('sale_id');
        $this->db->where('sale_id', $sale_id);
        $this->db->delete('sale');
    ...
    /* FUNCTION: Loads after successful benefitPay payment*/
    function benefitPay_success()
    {
        $carted = $this->cart->contents();
        $sale_id = $this->session->userdata('sale_id');
        $guest_id = $this->crud_model->get_type_name_by_id('sale', $sale_id, 'guest_id');
    ...
    function blog_view($para1 = "")
    {
        ...
        //)); ""Commented bcoz getting error in updating blog"
