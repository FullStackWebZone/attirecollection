...
class Crud_model extends CI_Model
{
    ...
    function get_type_name_by_id($type, $type_id = '', $field = 'name')
    {
		...
		if ($type_id == 'brand') {
            ...
            $l = $this->db->get_where($type);
			//print_r($this->db->last_query());
            $n = $l->num_rows();
            if ($n > 0) {
				$array1 = $l->result_array();
    ...
    function get_brands($type, $id, $vendor = '')
    {
        if ($type == 'category') {
            ...
            $sub_cats = array(array('sub_sub_category' => $id));
        } else if ($type == 'fourth_sub_category') {
            $sub_cats = array(array('fourth_sub_category' => $id));
        }
    ...
    function product_list_set($speciality, $limit, $id = '')
    {
        ...
        if ($speciality == 'sub_sub_category') {
            $this->db->where('sub_sub_category', $id);
        }

        if ($speciality == 'fourth_sub_category') {
            $this->db->where('fourth_sub_category', $id);
        }
    ...
    function if_publishable_subsubcategory($id)
    {
        $sub_sub_category_data = $this->db->get_where('sub_sub_category', array('sub_sub_category_id' => $id));
        $physical_product_activation = $this->db->get_where('general_settings', array('type' => 'physical_product_activation'))->row()->value;
        $digital_product_activation = $this->db->get_where('general_settings', array('type' => 'digital_product_activation'))->row()->value;

        if ($sub_sub_category_data->row()->digital == '') {
            if ($physical_product_activation !== 'ok') {
                return false;
            }
        } else if ($sub_sub_category_data->row()->digital == 'ok') {
            if ($digital_product_activation !== 'ok') {
                return false;
            }
        }
     ...
     function if_publishable_fourthsubcategory($id)
    {
        $fourth_sub_category_data = $this->db->get_where('fourth_sub_category', array('fourth_sub_category_id' => $id));
        $physical_product_activation = $this->db->get_where('general_settings', array('type' => 'physical_product_activation'))->row()->value;
        $digital_product_activation = $this->db->get_where('general_settings', array('type' => 'digital_product_activation'))->row()->value;

        if ($fourth_sub_category_data->row()->digital == '') {
            if ($physical_product_activation !== 'ok') {
                return false;
            }
        } else if ($fourth_sub_category_data->row()->digital == 'ok') {
            if ($digital_product_activation !== 'ok') {
                return false;
            }
        }
...
>
    ...
    */
	...
	function get_shipping_list(){
		$this->db->select('*');
		$this->db->from('country_shipping_charges');
		$this->db->join('country', 'country.country_id = country_shipping_charges.country_id');
		$query = $this->db->get()->result_array();
		return $query;
	}
	
	function get_shipping_list_by_productid($pid){
		$this->db->select('*');
		$this->db->from('country_shipping_charges');
		$this->db->join('country', 'country.country_id = country_shipping_charges.country_id');
	...
	function getUserVisitorCount(){
		$this->db->select('id,count,created_at');
